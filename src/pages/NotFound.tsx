    ...
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-gray-100">
      <header className="bg-gradient-to-r from-indigo-900 to-indigo-800 text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex gap-4">
            <Link to="/" className="hover:text-indigo-300 transition">Inicio</Link>
            {user ? (
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-indigo-700/50">
            <div className="max-w-7xl mx-auto px-4 py-3 flex flex-col gap-2">
              <Link to="/" className="block px-4 py-2 rounded-lg hover:bg-indigo-700 transition" onClick={() => setMenuOpen(false)}>Inicio</Link>
              {user ? (
                <Link to="/dashboard" className="block px-4 py-2 rounded-lg hover:bg-indigo-700 transition" onClick={() => setMenuOpen(false)}>Dashboard</Link>
              ) : (
                <Link to="/login" className="block px-4 py-2 rounded-lg hover:bg-indigo-700 transition" onClick={() => setMenuOpen(false)}>Iniciar Sesión</Link>
              )}
            </div>
          </div>
        )}
      ...
      <main className="flex items-center justify-center px-4 py-20">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-7xl font-bold text-indigo-900 mb-2">404</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              to="/"
              className="bg-indigo-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-indigo-500 transition shadow-md"
            >
              Volver al Inicio
            </Link>
            <Link
